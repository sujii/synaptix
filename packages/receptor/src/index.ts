...
app.post('/observe', async (req, res) => {
  ...
  const s = sent(out),
    m = meta(out),
    t = toks(out);
  const lexScore = Math.min(1, (s > 4 ? 0.34 : 0) + (m > 1 ? 0.33 : 0) + (t > 900 ? 0.33 : 0));
  ...
  const latScore = ewmaLatency > 3500 ? Math.min(1, (ewmaLatency - 3500) / 3500) : 0;

  // Fusion
  const score = Number((lexScore * 0.4 + polScore * 0.4 + latScore * 0.2).toFixed(3));
